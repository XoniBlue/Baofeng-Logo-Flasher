...
class TestBackupBytes:
    """Test LogoPatcher.backup_bytes method."""

    def test_backup_bytes_creates_file(self):
        """backup_bytes should create a file with the provided data."""
        import tempfile
        from pathlib import Path
        from baofeng_logo_flasher.logo_patcher import LogoPatcher

        with tempfile.TemporaryDirectory() as tmpdir:
            patcher = LogoPatcher(backup_dir=Path(tmpdir))

            test_data = b"test clone data 12345"
            result = patcher.backup_bytes(name="test_backup", data=test_data)
            ...
            assert backup_path.read_bytes() == test_data

    def test_backup_bytes_sanitizes_name(self):
        """backup_bytes should sanitize filename."""
        import tempfile
        from pathlib import Path
        from baofeng_logo_flasher.logo_patcher import LogoPatcher
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = patcher.backup_bytes(
                name="clone (model/version)",
                data=b"data"
            )

            backup_path = Path(result['path'])
            assert backup_path.exists()
            # Name should be sanitized (no parens, slashes)
