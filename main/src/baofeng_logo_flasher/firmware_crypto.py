...
# Encryption keys from reverse engineering
XOR_KEY1 = b"KDHT"
XOR_KEY2 = b"RBGI"
...
def xor_crypt(data: bytes, key: bytes) -> bytes:
    ...
    for i, byte in enumerate(data):
        key_byte = key[i % 4]
        # Skip encryption for special bytes
        if byte == 0x00 or byte == 0xFF or byte == key_byte or byte == (key_byte ^ 0xFF):
            result.append(byte)
        else:
...
def crypt_firmware(data: bytes, key1: bytes = XOR_KEY1, key2: bytes = XOR_KEY2) -> bytes:
    ...
    """
    packages = len(data) // PACKAGE_SIZE
    if len(data) % PACKAGE_SIZE > 0:
        packages += 1

    # Start with first 2 packages unencrypted
    result = bytearray(data[:2 * PACKAGE_SIZE])

    for i in range(2, packages):
        start = i * PACKAGE_SIZE
        end = min(start + PACKAGE_SIZE, len(data))
        block = data[start:end]

        # Last 2 packages are plaintext
        if i >= packages - 2:
            result.extend(block)
        # Apply encryption based on package position in 3KB cycle
        elif i % 3 == 1:
            result.extend(xor_crypt(block, key1))
        elif i % 3 == 2:
            result.extend(xor_crypt(block, key2))
        else:
            # i % 3 == 0: plaintext
            result.extend(block)
