...
| `src/baofeng_logo_flasher/__init__.py` | Package metadata/re-exports | `__version__`, `UV5RMTransport`, `UV5RMProtocol` | Importers | Imports protocol symbols at package import time |
...
| `src/baofeng_logo_flasher/boot_logo.py` | Boot logo domain logic | `flash_logo`, `read_radio_id`, `SERIAL_FLASH_CONFIGS` | CLI/UI/core actions | A5 protocol dispatch and serial identification |
...
| `src/baofeng_logo_flasher/protocol/uv5rm_protocol.py` | UV5R protocol operations | `UV5RMProtocol`, `RadioModel` | CLI detect/capabilities helpers | Reads radio metadata/version bytes |
...
| `src/baofeng_logo_flasher/core/actions.py` | Shared operation wrappers | `prepare_logo_bytes`, `flash_logo_serial` | CLI/UI integrations | Executes A5 flash flow + captures logs |
